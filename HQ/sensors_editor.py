    ...
    def save_data(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if sensor_id_item:
                ...
                # save active channel data

                if self.active_channel not in new_data:
                    new_data[self.active_channel] = {}
                new_data[self.active_channel][sensor_id] = {
                    "pan": float(safe_text(pan_item)) if safe_text(pan_item).lstrip('-').replace('.','',1).isdigit() else 0,
                    "tilt": float(safe_text(tilt_item)) if safe_text(tilt_item).lstrip('-').replace('.','',1).isdigit() else 0,
                }
